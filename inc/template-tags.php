...
if ( !function_exists( 'twentyfifteen_entry_meta' ) ) :
	...
	function twentyfifteen_entry_meta() {
		...
		if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
			...
			printf( '<p class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></p>', _x( 'Posted on', 'Used before publish date.', 'twentyfifteen' ), esc_url( get_permalink() ), $time_string
			);
		}
...
if ( !function_exists( 'twentyfifteen_excerpt_more' ) && !is_admin() ) :
	...
	function twentyfifteen_excerpt_more( $more ) {
		...
		/* translators: %s: Name of current post */ sprintf( __( 'Continue reading %s', 'twentyfifteen' ), '<span class="screen-reader-text">' . get_the_title( get_the_ID() ) . '</span>' )
		);
		return ' &hellip; ' . $link;
	}
